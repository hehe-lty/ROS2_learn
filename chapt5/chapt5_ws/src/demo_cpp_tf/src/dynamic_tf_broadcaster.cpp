...
#include "tf2_ros/transform_broadcaster.h" //坐标变换广播器
...
{
private:
    std::shared_ptr<tf2_ros::TransformBroadcaster> broadcaster_; //坐标变换广播器
...
public:
    TFBroadcaster() : Node("tf_broadcaster")
    {
        ...
        // this->publish_tf(); //发布坐标变换
        timer_ = this->create_wall_timer(100ms, std::bind(&TFBroadcaster::publish_tf, this)); //创建定时器，每100ms调用一次publish_tf函数
        RCLCPP_INFO(this->get_logger(), "发布map坐标系到base_link坐标系的坐标变换");
        // RCLCPP_INFO(this->get_logger(), "发布map坐标系到target_point坐标系的坐标变换");
    }
